...
import serendip.struts.plugins.thymeleaf.TymeleafBHMessageSource;
...
public class DefaultTemplateEngineProvider implements TemplateEngineProvider {
	...
	public void configure() {
		...
		MessageSource mm= messageSource() ;
		templateEngine.setTemplateResolver(templateResolver);
		//templateEngine.setMessageResolver(new StrutsMessageResolver());
		templateEngine.setMessageResolver(messageResolver(mm));
		SpringTemplateEngine pTemplateEngine=(SpringTemplateEngine)templateEngine ;
		pTemplateEngine.setMessageSource(mm);
		// extension diarects.
	...
	public MessageSource messageSource()
	{
		...
		TymeleafBHMessageSource messageSource = new TymeleafBHMessageSource();
		return messageSource;
	}
	public IMessageResolver messageResolver(MessageSource mSource) {
		SpringMessageResolver messageResolver = new SpringMessageResolver();
		messageResolver.setMessageSource(mSource);
//		messageResolver.setOrder(1);
	...
	public void setContainer(Container container) {
		...
		for (String prefix : prefixes) {
			TemplateEngine engine = (TemplateEngine) container.getInstance(TemplateEngine.class, prefix);//new TemplateEngine
//			TemplateEngine engine = (TemplateEngine) container.getInstance(SpringTemplateEngine.class, prefix);//new TemplateEngine
